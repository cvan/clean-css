...
var CleanCSS = {
  ...
  process: function(data) {
    ...
    replace(/([: ,=\-])0*(\.?\d)/g, '$1$2') // strip leading zeros in fractions
    replace(/([: ,=\-])\.([\d^0]+)0/g, '$1.$2') // strip trailing zeros in fractions
    ...
    replace(/(margin|padding|border-width):([^;\}]+)/g, function(match, property, values) {
      values = values.split(' ');
      ...
      if (t == r && t == b && t == l) {
        ...
        // 5px 10px 7px 10px => 5px 10px 7px
        result = [t, r, b];
      }
      return property + ':' + result.join(' ');
    })
